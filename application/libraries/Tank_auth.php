...
class Tank_auth
{
	...
	function logout()
	{
		...
		$this->ci->session->set_userdata(array('user_id' => '', 'username' => '', 'capabilities' => '', 'status' => ''));
	...
	{
		$apaaja = $this->ci->session->userdata('capabilities');
		if (empty($apaaja)) {
			$apaaja = serialize(array());
		}
		return $apaaja;
	 ...
	 * @return	array
	 */
	function create_user($username, $email, $password, $email_activation, $role = 1, $cap = array())
	{
